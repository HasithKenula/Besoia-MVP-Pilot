export default function MatchResultModal({ match, onClose, onViewResult }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-emerald-950/70 p-5" role="dialog" aria-modal="true" aria-labelledby="match-result-title">
    <div className="w-full max-w-lg rounded-2xl bg-stone-100 p-7 text-center shadow-2xl sm:p-10">
      ...
      <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:justify-center">
        <button type="button" className="rounded bg-emerald-950 px-5 py-3 text-stone-100" onClick={onViewResult}>View full result</button>
        <button type="button" className="rounded border border-emerald-950 px-5 py-3" onClick={onClose}>Close</button>
      </div>
    </div>
  </div>;
}
